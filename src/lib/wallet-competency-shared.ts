...
export function countWalletEvidence(params: {
  ...
  return (
    ...
    + (params.externalEvidence?.length ?? 0)
  );
}
...
export function buildWalletEvidenceSummary(params: {
  ...
  timestampGroups: {
    ...
    externalEvidence?: Array<string | null | undefined>;
  };
  ...
  return {
    ...
    evidenceTimestamps: {
      ...
      externalEvidence: uniqueTimestamps(params.timestampGroups.externalEvidence ?? []),
    },
    sourceBadges,
    ...
    status: {
      taskStatus: practicalTaskStatus,
      reviewStatus: params.peerReviews.length + params.teacherFeedback.length > 0 ? "Review Available" : "Pending Review",
      verificationStatus: "Unverified",
      walletStatus: deriveWalletRecordStatus({
        githubCount:
          params.github.repos.length
          + params.github.activities.length
          + params.github.evidenceRecords.length
        ...
        lmsCount:
          ...
          + (params.externalEvidence?.length ?? 0),
        practicalTaskStatus,
        peerReviewCount: params.peerReviews.length + params.teacherFeedback.length,
      }),
    },
    metadata: {
